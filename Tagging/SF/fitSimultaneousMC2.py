...
ftemplate = {
      'pass' : root.TFile(basedir+'tag_pass_hists.root'),
      'fail' : root.TFile(basedir+'tag_fail_hists.root'),
    }

inputs = {
    1 : ['QCD','Z+g','Z+q','W+g','W+q','Top [unmatched]','Diboson', 'Top [W-matched]'],
    3 : ['Top [t-matched]'],
    }
...
def build_sum_hist(cat,nprong):
  ...
  for i in inputs[nprong]:
    if not hsum:
      hsum = f.Get('h_fj1MSD_%s'%(i)).Clone('h_%s_%i'%(cat,nprong))
    else:
      hsum.Add(f.Get('h_fj1MSD_%s'%(i)))
  return hsum
...
#                            root.RooArgList(*[pdfprong[(x,iC)] for x in [1,2,3]]),
                            root.RooArgList(*[smeared[(x,iC)] for x in [1,3]]),
                            root.RooArgList(norm[(1,iC)],normsig[iC]))
...
for iC in [0,1]:
  ...
  hmodel.Scale(sum([norm[(x,iC)].getVal() for x in [1,3]])/hmodel.Integral())
...
plot[1].Draw(basedir,'/fits/pass%s'%masscorr)
plot[0].Draw(basedir,'/fits/fail%s'%masscorr)
