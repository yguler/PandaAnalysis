...
parser.add_argument('--era',metavar='era',type=str,default=None)
...
if args.era and args.era!="ALL":
    run_boundaries = []
    lumi = 0
    for e in args.era:
        era = all_eras[e]
        run_boundaries += [era.lo,era.hi]
        lumi += era.lumi
    datacut = 'runNumber>=%i && runNumber<=%i'%(min(run_boundaries),max(run_boundaries))
...
if ('signal' in region) and blind and False:
    ...
    if args.era and args.era!="ALL":
        plot.AddPlotLabel('Run2016%s'%(args.era),.18,.7,False,42,0.04)
...
if args.era and args.era!='ALL':
    data.additional_cut = tAND(data.additional_cut,datacut)
    if region=='photon':
        qcd.additional_cut = tAND(qcd.additional_cut,datacut)
